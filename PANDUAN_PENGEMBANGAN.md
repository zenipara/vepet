...
> **Versi Dokumen:** 1.0.0  
> **Status:** Production-Ready Development Guide  
> **Target:** Developer / GitHub Copilot / Codespace  
...
6. [Database Schema Lengkap](#6-database-schema-lengkap)
7. [Supabase RLS & Security](#7-supabase-rls--security)
8. [Sistem Autentikasi & Role](#8-sistem-autentikasi--role)
9. [Feature Modules](#9-feature-modules)
10. [Dashboard per Role](#10-dashboard-per-role)
11. [Realtime Engine](#11-realtime-engine)
12. [CMS System](#12-cms-system)
13. [Inventory & Business Logic](#13-inventory--business-logic)
14. [CI/CD & Deployment](#14-cicd--deployment)
15. [Konvensi Kode](#15-konvensi-kode)
16. [Anti-Pattern & Best Practice](#16-anti-pattern--best-practice)
17. [Roadmap Pengembangan](#17-roadmap-pengembangan)
18. [FAQ Developer](#18-faq-developer)

---

## 1. Gambaran Sistem

**VetCare System** adalah platform manajemen klinik hewan dan petshop berbasis web yang mengintegrasikan:

- Operasional klinik harian (appointment, check-in, queue)
- Electronic Medical Record (EMR) pasien hewan
- Recovery Journey real-time untuk transparansi ke pemilik hewan
- Manajemen inventaris obat & produk
- CMS publik tanpa redeploy
- SaaS-ready multi-klinik (future)

### Tujuan Utama

| No | Tujuan |
|----|--------|
| 1 | Digitalisasi operasional klinik hewan end-to-end |
| 2 | Transparansi kondisi hewan kepada pemilik secara real-time |
| 3 | Efisiensi kerja dokter, staf, dan admin |
| 4 | Manajemen stok otomatis & terintegrasi dengan treatment |
| 5 | Fondasi SaaS multi-klinik yang scalable |
...
├── Realtime Engine     → Live updates via WebSocket
├── Storage             → Upload foto & dokumen
└── Edge Functions      → Logic kompleks (Deno runtime)
```
...
|-------|----------|--------|
| Frontend | GitHub Pages | Static SPA (`/dist`) |
| Backend | Supabase Cloud | Managed PostgreSQL + API |
...
│  ┌────────────────────────────────────────────────┐  │
│  │              Supabase Auth                      │  │
│  │   JWT Token → Role Fetch → Redirect Dashboard  │  │
│  └─────────────────────┬──────────────────────────┘  │
│                        │                              │
│  ┌─────────────────────▼──────────────────────────┐  │
│  │           PostgreSQL Database                   │  │
...
│  ┌──────▼──────┐          ┌────────▼──────┐          │
│  │  Realtime   │          │    Storage    │          │
│  │  WebSocket  │          │  Photos/Docs  │          │
│  └─────────────┘          └───────────────┘          │
│                                                       │
│  ┌────────────────────────────────────────────────┐  │
│  │              Edge Functions (Deno)              │  │
│  │   booking / inventory / notifications           │  │
│  └────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────┘
```
